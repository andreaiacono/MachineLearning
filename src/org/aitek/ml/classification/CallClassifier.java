...
/*
 * Example class which creates a classifier and evaluates it via cross- validation (or test set),
 * outputting the complete class probability distributions
 */
...
public class CallClassifier {
	...
	public static void main(String[] argv) {
		...
		try {
			...
			String trainFile = "resources/tweets_train.arff";
			...
			if (testFile.length() != 0) testData = new Instances(new FileReader(testFile));
			...
			if (testData == null) {
				...
				for (int i = 0; i < numFolds; i++) {
					...
					for (int j = 0; j < testData.numInstances(); j++) {
						...
						double predValue = c.classifyInstance(instance);
						...
						if (testData.classAttribute().isNumeric()) {
							...
							if (Instance.isMissingValue(predValue)) {
								...
								System.out.print(idx + delim + testData.classAttribute().value((int) predValue) + delim);
							}
							if (Instance.isMissingValue(predValue)) {
							...
							/*
							 * Note: the order of class probabilities corresponds to the order of
							 * class values in the training file
							 */
							System.out.print("\n");
						}
					}
				...
				for (int i = 0; i < testData.numInstances(); i++) {
					...
					if (testData.classAttribute().isNumeric()) {
						...
						for (int k = 0; k < dist.length; k++)
							System.out.print(delim + dist[k]);
						/*
						 * Note: the order of class probabilities corresponds to the order of class
						 * values in the training file
						 */
						System.out.print("\n");
					}
				}
